...
Drupal.galleryformatter.prepare = function(el) {
  (function ($) {
	...
	// get set a few simple slideshow vars (just 1 presently anything over '0' = enabled)
	var $gfslideshowSpeed = 4000;
 ...
 //custom addition by ashish=======================   
    $thumbslinks.mouseover(function(e){
        $hash = $(this.hash);
        if(!$hash.is(':visible')){
          $thumbsLi.removeClass('active');
          $(this).parent().addClass('active');
...
//custom addition by ashish ends =======================
...
/* $tart testing Gallery Formatter Slideshow - DRIVE 08082011 $ */
if ($gfslideshowSpeed != '0') {
	//$wrapper.after('<a id="gfslideshowbutton" class="arrow gfslideshow" title="'+ Drupal.t('Auto') +'">Auto</a>');
	//alert('Gallery Formatter Slideshow Enabled: ' + $gfslideshowSpeed);
	autoscrolling = true;
	/*$('a#gfslideshowbutton', $el).click(function(){
		autoscrolling = true;
		$('a#gfslideshowbutton', $el).removeClass("gfslideshow").addClass("gfslideshowon");
	});*/
/* pause/resume slideshow w/mouse needs work + moved - fires indiscriminately at present*/
$thumbs.trigger('showNext').mouseover(function(){
	autoscrolling = false;
}).mouseout(function () {
	autoscrolling = true;
});

$slides.trigger('showNext').mouseover(function(){
	autoscrolling = false;
}).mouseout(function () {
	autoscrolling = true;
});
// Slides and thumbs scroll in sync
	setInterval(function () {
		if (autoscrolling) {
			$thumbs.trigger('showNext');
		}
	}, $gfslideshowSpeed); // 0btained from settings @ 
// Structure --> Content Types --> [Your content type] --> 
// Manage Display[image field you are using for Gallery Formatter]
}
